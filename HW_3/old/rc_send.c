...
static void usage(const char *argv0)
{
	...
	printf("  -r, --rkey=<rkey>  remote key of destination buffer\n");
}
...
int main(int argc, char *argv[]) {
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "p:d:i:g:q:l:r:",
				long_options, NULL);
		if (c == -1)
			break;
		...
		case 'q':
			...
			break;
		case 'r':
			dest_rkey = strtol(optarg, NULL, 0);
			if (dest_rkey < 0) {
				usage(argv[0]);
				return 1;
			}
